...
func (g GormRepository) UserCountingOrder(userId string) (int, error) {
	var count int64
...
func (g GormRepository) StoreCountingOrder(storeId string) (int, error) {
	queryResp := struct {
	...
	sql := `SELECT count(distinct order_id) as count
			...
			Where store_id = ?
			`
	err := g.client.DB().Raw(sql, storeId).Scan(&queryResp).Error
	if err != nil {
		return 0, err
	}
...
func (g GormRepository) DeliveryCountingOrder(deliveryId string) (int, error) {
	var count int64
	result := g.client.DB().Model(&entity.Delivery{}).
...
func (g GormRepository) AdminCountingOrder() (int, error) {
	var count int64
